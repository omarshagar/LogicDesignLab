...
/*
    @Override
    public void drag(Yard yar,FPin fp) {
        if(yar.dragSystem.x1==-1)
                {
                    //feqy please do connectevity staff between this FPin and c wire pins
                    this.drawFirstPin(fp.getRowLocInPanel(), fp.getColLocInPanel(), fp.getCurRowInBB(), fp.getCurColInBB());
                    yar.dragSystem.x1=fp.getColLocInPanel();
                   
                    yar.dragSystem.y1=fp.getRowLocInPanel();
                }
                else 
                {
                     //feqy please do connectevity staff between this FPin and c wire pins
                    this.drawSecPin(fp.getRowLocInPanel(), fp.getColLocInPanel(), fp.getCurRowInBB(), fp.getCurColInBB());
                    yar.dragSystem.x2=fp.getColLocInPanel();
                   
                    yar.dragSystem.y2=fp.getRowLocInPanel();
             
                   
                    yar.dragSystem.clean();
                    yar.itemselected.setText(" ");
